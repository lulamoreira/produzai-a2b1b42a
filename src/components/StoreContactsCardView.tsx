  ...
  return (
    <div className="space-y-4">
      <div className="relative max-w-md">
        ...
        <Input
          placeholder={t("stores.searchContactPlaceholder")}
          value={search}
      ...
      <p className="text-xs text-muted-foreground">
        {filteredContacts.length} {t("stores.contactCountShort")} em {groupedByStore.length} {t("stores.storeCount")}
      </p>

      {groupedByStore.length === 0 ? (
        <div className="text-center py-12">
          <UserPlus className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-muted-foreground text-sm">{t("stores.noContactFound")}</p>
        </div>
      ) : (
        <div className="space-y-4">
