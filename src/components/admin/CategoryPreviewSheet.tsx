/**
 * CategoryPreviewSheet — read-only visual map of what a category authorises.
 *
 * Mounted inside the already-authenticated admin context as a Sheet.
 */
...
import {
  Check, X, Eye, Pencil, Trash2, Sparkles, Lock,
} from "lucide-react";
...
export function CategoryPreviewSheet({ open, onOpenChange, categoryId }: Props) {
  const isMobile = useIsMobile();

  const { data: category, isLoading: loadingCat } = useQuery({
    queryKey: ["permission_category", categoryId],
  ...
  const { data: grants = [], isLoading: loadingGrants } = useCategoryGrants(open ? categoryId ?? undefined : undefined);
  ...
  const sheetClass = isMobile
    ? "h-[90vh] max-h-[90vh] overflow-y-auto p-0"
    : "w-full sm:max-w-4xl overflow-y-auto p-0";

  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side={sheetSide} className={sheetClass}>
        {(loadingCat || loadingGrants) ? (
          ...
          <>
            <header className="border-b border-border bg-card sticky top-0 z-10 px-6 py-4">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Visualização das permissões — o que esta categoria autoriza no sistema
              </div>
            </header>
