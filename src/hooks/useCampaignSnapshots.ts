  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const storePiecesAll = await supabasePaginate<any>((from, to) =>
        sb.from("campaign_store_pieces").select("*", { count: "exact" }).eq("campaign_id", campaignId!).order("id").range(from, to)
      );
...
export function useCreateSnapshot() {
  ...
  return useMutation({
    mutationFn: async (input: { campaign_id: string; name: string; description?: string }) => {
      ...
      const storePiecesAll = await supabasePaginate<any>((from, to) =>
        supabase.from("campaign_store_pieces").select("*", { count: "exact" }).eq("campaign_id", campaign_id).order("id").range(from, to) as any
      );
      const storePiecesRes = { data: storePiecesAll, error: null as any };
      if (piecesRes.error) throw piecesRes.error;
      if (kitsRes.error) throw kitsRes.error;
