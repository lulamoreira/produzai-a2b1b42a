...
- Cada peça é um CÓDIGO DE ARQUIVO no formato LINDT_<PAÍS>_<CAMPANHA>_<LOCALIZAÇÃO>_<SUBGRUPO>_<TIPO...>_<TAMANHO>. O "Nome da Peça" deve ser um NOME CURTO E LEGÍVEL, no estilo do próprio cliente, derivado do código. Para montá-lo: remova o prefixo fixo de cliente/país/campanha (ex.: LINDT, CH, NAVIDAD); remova o ANO (ex.: 2019, 2020, 2021); remova a MEDIDA/dimensões do final (ela já vai para "Tamanho da Peça"); remova tokens que apenas repetem a seção/subseção de localização (ex.: VIT, INT, QUIOSCO, TDS quando forem só o marcador de localização). Converta os tokens em MAIÚSCULAS_COM_UNDERLINE para palavras em Title Case com espaços. MANTENHA a parte descritiva e as variações (ex.: TAG, SELL OFF, WALLBAY, 30%, 50%, G/M/P, PRIM/SEC). Exemplo: "LINDT_CH_NAVIDAD_TAG_SELL_OFF_30%_12x12cm" → "Tag Sell Off 30%". NÃO mantenha o código cru como nome. Quando o item claramente corresponder a um nome já usado pelo cliente (lista de aprendizado abaixo, se houver), reutilize esse nome exatamente.
