...
grant execute on function public.clone_campaign_for_renegotiation(uuid) to authenticated;

-- BACKFILL: move favoritos antigos para a versao mais recente de cada cadeia
insert into user_campaign_favorites (user_id, campaign_id)
select distinct f.user_id, l.latest_id
from user_campaign_favorites f
join (select id, coalesce(root_campaign_id, id) as root from campaigns) ch on ch.id = f.campaign_id
join (
  select coalesce(root_campaign_id, id) as root, (array_agg(id order by created_at desc, id desc))[1] as latest_id
  from campaigns group by coalesce(root_campaign_id, id)
) l on l.root = ch.root
where f.campaign_id <> l.latest_id
  and not exists (select 1 from user_campaign_favorites x where x.user_id = f.user_id and x.campaign_id = l.latest_id);

delete from user_campaign_favorites f
using (select id, coalesce(root_campaign_id, id) as root from campaigns) ch,
      (select coalesce(root_campaign_id, id) as root, (array_agg(id order by created_at desc, id desc))[1] as latest_id
       from campaigns group by coalesce(root_campaign_id, id)) l
