-- 1) Tables
CREATE TABLE IF NOT EXISTS public.store_import_batches (
...
CREATE TABLE IF NOT EXISTS public.store_import_snapshot_items (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_store_import_snapshot_items_batch_id ON public.store_import_snapshot_items(batch_id);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own client''s import batches') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own client''s import snapshot items') THEN
