...
DECLARE
  result jsonb;
BEGIN
  -- Access gate: user must have access to at least one campaign of this client
  IF NOT EXISTS (
...
REVOKE ALL ON FUNCTION public.get_client_teams_for_import(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_client_teams_for_import(uuid) TO authenticated, service_role;
