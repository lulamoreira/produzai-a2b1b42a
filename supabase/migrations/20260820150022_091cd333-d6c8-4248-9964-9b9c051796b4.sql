...
declare
  ...
  v_base text;
begin
  -- CORREÇÃO: Ordem dos argumentos em has_campaign_access(_user_id, _campaign_id)
  ...
  v_seq := v_seq + 1;
  if v_seq = 1 then v_label := 'RENEGOCIACAO INICIAL';
  else v_label := 'RENEGOCIACAO ' || v_seq::text;
  end if;
  ...
  select gen_random_uuid(), v_new, budget_amount, currency_code, currency_locked, current_phase, deadline,
    negotiation_mode, negotiation_target, notify_user_ids, phase_locked_at, null, null, null
  from budget_settings where campaign_id=_source_campaign_id;

  insert into budget_extra_costs (id, supplier_id, installation_value, freight_value, adjusted_installation_value, adjusted_freight_value)
  select gen_random_uuid(), ms.new_id, ec.installation_value, ec.freight_value, ec.adjusted_installation_value, ec.adjusted_freight_value
